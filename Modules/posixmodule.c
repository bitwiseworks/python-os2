...
#if defined(__FreeBSD__) || defined(__DragonFly__) || defined(__APPLE__) || defined(__OS2__)
...
parse_envlist(PyObject* env, Py_ssize_t *envc_ptr,
              EXECV_CHAR **sp_envvars, ULONG *sp_envids, Py_ssize_t *sp_envc)
#else
parse_envlist(PyObject* env, Py_ssize_t *envc_ptr)
#endif
{
    ...
    PyObject *key2, *val2, *keyval=NULL;
    EXECV_CHAR **envlist;
#ifdef __OS2__
    ULONG envid;

    memset(sp_envvars, 0, OS2_SPECIAL_ENVVAR_CNT * sizeof(*sp_envvars));
    memset(sp_envids, 0, OS2_SPECIAL_ENVVAR_CNT * sizeof(*sp_envids));
    *sp_envc = 0;
#endif
...
#if (defined(HAVE_WEXECV) || defined(HAVE_WSPAWNV)) && !defined(__OS2__)
...
#ifdef __OS2__
        envid = os2_envvar_special(PyBytes_AS_STRING(key2));
        if (envid) {
            ...
            EXECV_CHAR **oldval = sp_envvars[*sp_envc] ? NULL : &sp_envvars[*sp_envc];
            sp_envids[*sp_envc] = envid;
            if (os2_putenv_special(envid, PyBytes_AS_STRING(val2), oldval)) {
                if (oldval && *oldval) {
                    PyMem_Free(*oldval);
                    *oldval = NULL;
                }
...
#ifdef __OS2__
    EXECV_CHAR *sp_envvars[OS2_SPECIAL_ENVVAR_CNT];
    ULONG sp_envids[OS2_SPECIAL_ENVVAR_CNT];
    Py_ssize_t sp_envc;
#endif
...
#ifdef __OS2__
    envlist = parse_envlist(env, &envc, sp_envvars, sp_envids, &sp_envc);
#else
...
#if !defined(HAVE_RTPSPAWN) && !defined(__OS2__)
...
#ifdef __OS2__
    envid = os2_envvar_special(name_string);
    if (envid) {
        if (os2_putenv_special(envid, value_string, NULL))
            return NULL;
    } else {
#endif
...
#if defined(HAVE_SPAWNV) && defined(__OS2__)
    if (PyModule_AddIntConstant(m, "P_WAIT", P_WAIT)) return -1;
    if (PyModule_AddIntConstant(m, "P_NOWAIT", P_NOWAIT)) return -1;
    if (PyModule_AddIntConstant(m, "P_OVERLAY", P_OVERLAY)) return -1;
    if (PyModule_AddIntConstant(m, "P_DEBUG", P_DEBUG)) return -1;
    if (PyModule_AddIntConstant(m, "P_SESSION", P_SESSION)) return -1;
    if (PyModule_AddIntConstant(m, "P_DETACH", P_DETACH)) return -1;
    if (PyModule_AddIntConstant(m, "P_PM", P_PM)) return -1;
    if (PyModule_AddIntConstant(m, "P_DEFAULT", P_DEFAULT)) return -1;
    if (PyModule_AddIntConstant(m, "P_MINIMIZE", P_MINIMIZE)) return -1;
    if (PyModule_AddIntConstant(m, "P_MAXIMIZE", P_MAXIMIZE)) return -1;
    if (PyModule_AddIntConstant(m, "P_FULLSCREEN", P_FULLSCREEN)) return -1;
    if (PyModule_AddIntConstant(m, "P_WINDOWED", P_WINDOWED)) return -1;
    if (PyModule_AddIntConstant(m, "P_FOREGROUND", P_FOREGROUND)) return -1;
    if (PyModule_AddIntConstant(m, "P_BACKGROUND", P_BACKGROUND)) return -1;
    if (PyModule_AddIntConstant(m, "P_NOCLOSE", P_NOCLOSE)) return -1;
    if (PyModule_AddIntConstant(m, "P_NOSESSION", P_NOSESSION)) return -1;
    if (PyModule_AddIntConstant(m, "P_QUOTE", P_QUOTE)) return -1;
    if (PyModule_AddIntConstant(m, "P_TILDE", P_TILDE)) return -1;
    if (PyModule_AddIntConstant(m, "P_UNRELATED", P_UNRELATED)) return -1;
    if (PyModule_AddIntConstant(m, "P_DEBUGDESC", P_DEBUGDESC)) return -1;
    if (PyModule_AddIntConstant(m, "P_2_NOINHERIT", P_2_NOINHERIT)) return -1;
    if (PyModule_AddIntConstant(m, "P_2_THREADSAFE", P_2_THREADSAFE)) return -1;
    if (PyModule_AddIntConstant(m, "P_2_MODE_MASK", P_2_MODE_MASK)) return -1;
#else
