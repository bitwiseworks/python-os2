...
# os_name           -- [in] one of 'nt', 'posix', 'darwin', 'os2'    SEP = '\\'
...
if os_name == 'os2' and not executable and '/' in program_name:
    # Resolve partial path program_name against current directory
    executable = abspath(program_name)
...
if not py_setpath and not home_was_set:
    ...
    for p in [library, executable, real_executable]:
        if p:
            if os_name in('nt', 'os2') and (hassuffix(p, 'exe') or hassuffix(p, 'dll')):
                p = p.rpartition('.')[0]
            p += '._pth'
            try:
                pth = readlines(p)
